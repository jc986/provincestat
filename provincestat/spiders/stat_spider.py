    ...
    def parse_province(self, response):
        ...
        for p in province_list:
            ...
            for td in td_list:
                item = AreaItem()
                # 个别td的内容为空
                if (td.xpath('./a').get(default='not-found') != 'not-found' or td.xpath('./text()').get(default='not-found') != 'not-found'):
                    ...
                    yield scrapy.Request(item['link'], callback=self.parse_city, cb_kwargs=dict(pitem=item))
        # return self.items

    def parse_city(self, response, pitem):
        '''
        解析市信息
        :param response:
        :param pitem:
